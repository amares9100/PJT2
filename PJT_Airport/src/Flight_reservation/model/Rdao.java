...
public class Rdao extends Dao{
	...
	public Reservation ticketReservation1(int sno, int mno) {
		...
		String sql = "select s.sno '스케쥴 번호' ,  al.lname '항공사명' , ap.aname '비행기명' ,  ap1.pname'출발지' , ap2.pname '도착지' , s.dtime '비행일' , s.atime '도착일' , s.price '가격' , s.rseats '남은좌석'  , m.mno '멤버번호' ,t_t.discount '할인율' ,t_t.arate '마일리지 적립비율' ,  m.mileage '멤버마일리지'    "
	...
	//멤버 마일리지
	public boolean mileagPoints(int mno) {
		String sql ="select Mileage from member where mno="+mno;
		
		try {
			ps=conn.prepareStatement(sql);
			rs = ps.executeQuery();
			String sql2 = "update  member  set  Mileage ="+rs.getInt(1)+" where mno="+mno;
